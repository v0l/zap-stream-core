...
impl CloudflareClient {
    ...
    pub fn new(key: CloudflareToken) -> Self {
        Self {
    ...
    pub async fn get_live_input(&self, uid: &str) -> Result<ApiResponse<LiveInput>> {
        let url = format!(
            "{}/accounts/{}/stream/live_inputs/{}",
    ...
    pub async fn get_live_input_outputs(
        &self,
        uid: &str,
    ) -> Result<ApiResponse<Vec<LiveInputOutput>>> {
        let url = format!(
    ...
    pub async fn create_live_input_output(
        &self,
        uid: &str,
        target: &str,
        key: &str,
    ) -> Result<ApiResponse<LiveInputOutput>> {
        let url = format!(
            "{}/accounts/{}/stream/live_inputs/{}/outputs",
            self.base_url, self.account_id, uid
        );
    ...
    pub async fn update_live_input_output(
        &self,
        uid: &str,
        output_id: &str,
        enabled: bool,
    ...
    pub async fn get_video_assets(
        &self,
        live_input_uid: &str,
    ) -> Result<ApiResponse<Vec<VideoAsset>>> {
        let url = format!(
            "{}/accounts/{}/stream?liveInput={}",
...
mod tests {
    ...
    async fn test_get_live_input_success() {
        let mut server = Server::new_async().await;
        let mock = server
            .mock("GET", "/accounts/test-account/stream/live_inputs/test-uid")
            .match_header("authorization", "Bearer test-token")
            .with_status(200)
            .with_header("content-type", "application/json")
            .with_body(
    ...
    async fn test_delete_live_input_success() {
        ...
        let mock = server
            .mock(
                "DELETE",
                "/accounts/test-account/stream/live_inputs/test-uid",
            )
            .match_header("authorization", "Bearer test-token")
            .with_status(200)
            .create_async()
