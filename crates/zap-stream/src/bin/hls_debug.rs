...
fn main() -> Result<()> {
    ...
    for segment_type in &playlist.segments {
        match segment_type {
            MediaSegmentType::Full(segment) => {
                ...
                let durations = if segment_path.extension().is_some_and(|ext| ext == "m4s") {
    ...
    for segment in &segments {
        ...
        if video_diff_ratio > 0.3 && video_dur < playlist_dur * 0.7 {
            validation_errors.push(format!(
                ...
                segment.filename,
                video_dur,
                (1.0 - video_dur / playlist_dur) * 100.0,
                playlist_dur
            ));
        }
        ...
        if audio_diff_ratio > 0.1 {
            let direction = if audio_dur > playlist_dur {
                "longer"
            } else {
                "shorter"
            };
    ...
    if !segments.is_empty() {
        let avg_video_dur: f64 =
            segments.iter().map(|s| s.video_duration).sum::<f64>() / segments.len() as f64;
        let avg_audio_dur: f64 =
            segments.iter().map(|s| s.audio_duration).sum::<f64>() / segments.len() as f64;
        let avg_playlist_dur: f64 = segments
            .iter()
            .map(|s| s.playlist_duration as f64)
            .sum::<f64>()
            / segments.len() as f64;
        ...
        let video_playlist_ratio = if avg_playlist_dur > 0.0 {
            avg_video_dur / avg_playlist_dur
        } else {
            0.0
        };
        let audio_playlist_ratio = if avg_playlist_dur > 0.0 {
            avg_audio_dur / avg_playlist_dur
        } else {
            0.0
        };
        ...
        if (audio_playlist_ratio - 1.0).abs() > 0.1 {
            let direction = if audio_playlist_ratio > 1.0 {
                "longer"
            } else {
                "shorter"
            };
