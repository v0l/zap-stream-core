...
pub struct ZapStreamOverseer {
    ...
    /// HLS segment length
    segment_length: f32,
    /// Event queue sender for background publishing
    event_queue_tx: mpsc::UnboundedSender<Event>,
}
...
impl ZapStreamOverseer {
    ...
    pub async fn new(
        ...
        // Create event queue for background publishing
        let (event_queue_tx, mut event_queue_rx) = mpsc::unbounded_channel::<Event>();
        
        // Spawn background task to process event queue
    ...
    pub async fn publish_stream_event(
        ...
        // Queue event for background publishing instead of blocking
        if let Err(e) = self.event_queue_tx.send(ev.clone()) {
            warn!("Failed to queue event for publishing: {}", e);
        }
...
impl Overseer for ZapStreamOverseer {
            ...
            if !is_active || should_timeout {
                ...
                if let Ok(user) = self.db.get_user(stream.user_id).await {
                    if self
                        ...
                        .await?
                    {
                        let _ = self.publish_stream_event(&stream, &user.pubkey).await;
                    }
                }
            }
        }
    ...
    async fn start_stream(
        ...
        if let Ok(stream_event) = self.publish_stream_event(&new_stream, &user.pubkey).await {
    ...
    async fn on_end(&self, pipeline_id: &Uuid) -> Result<()> {
        ...
        if let Ok(event) = self.publish_stream_event(&stream, &user.pubkey).await {
